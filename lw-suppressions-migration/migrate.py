...
class LPP:
    def __init__ (self, lwPolicyNumber, listOfConstraintTypes):
        ...
        self.listOfConstraintTypes = listOfConstraintTypes

# constraint types
#ResourceNames, in the old CIS1.1, works for EC2 instance ID, VPC ID, Group ID, ARN, ELB ID, Lambda name, IAM policy, etc)
...
"AWS_CIS_2_3" : LPP("54",noReg_c), #s3 bucket cloutrail log
"AWS_CIS_2_4" : LPP("55",noTag_c),
"AWS_CIS_2_5" : LPP("76",noTag_c),
"AWS_CIS_2_6" : LPP("56",noReg_c), #s3 bucket cloutrail log
...
def createPayload (awssupppresions):
    ...
    for k,v in awssupppresions.items():
        ...
        #ignore enable/disabled, we want to migrate the suppresions, not the enablement of the policy
        ...
        for suppressionCondition in v["suppressionConditions"]: #handle multiple suppressions per policy 
            ...
            for fieldKey,value in suppressionCondition.items(): 
                ...
                if fieldKey == "comments":
                    ...
                    if len(value)>0:
                        logging.info("# LPP Policy " + LPPPolicy + " does not support the suppression condition "+fieldKey + ". Discarding...")
                        discardedSuppressions.append((LPPPolicy,fieldKey,value))
            ...
            logging.info("# Creating Exception migration num "+ str(suppcount) +"for a suppresion that had constraint types: " + str(equivalences_map[k].listOfConstraintTypes))
            lwapitext = "lacework api post '/Exceptions?policyId=" + LPPPolicy + "' -d '" + json.dumps(payload) + "'"
            print (lwapitext+"\n")
            payloadsText.append(lwapitext)
            suppcount+=1
